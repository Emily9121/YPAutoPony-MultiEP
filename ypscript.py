...
with open("config") as f:
    ...
    for line in f:
        ...
        for fpath in fpaths:
            ...
            rawnum = fname.split(None, 1)[0]
			# split all the episodes number of the file
            allepisodename = ""
            for atomnum in rawnum.split("_"):
                try:
                    epnum = int(atomnum)
                except:
                ...
                episode = tvdb['Star vs The Forces of Evil'][options['SEASON']][epnum]
                toadd = "S"+('%0.2d' % options['SEASON']) + "E" + atomnum.zfill(2) + " : " + episode['episodename']
                if allepisodename == "":
                    allepisodename = toadd
                else:
                    allepisodename = allepisodename + " - " + toadd
